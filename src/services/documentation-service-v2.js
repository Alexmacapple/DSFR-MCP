...
class DocumentationServiceV2 extends IService {
  ...
  async getComponentDetails({ component_name, include_examples = true, include_accessibility = true }) {
    ...
    try {
      ...
      const formattedText = this.formatComponentDetails(component, include_examples, include_accessibility);
  ...
  async getIcons({ category, search }) {
    ...
    try {
      ...
      const iconResults = searchResult.results.filter(result => 
  ...
  async getColors({ include_utilities = true, format = 'hex' }) {
    ...
    try {
      ...
      const formattedText = this.formatColorResults(searchResult.results, include_utilities, format);
