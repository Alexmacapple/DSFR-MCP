...
class DSFRIconDatabase {
  ...
  searchIcons(query = '', category = null, limit = 20) {
    ...
    if (category && category !== 'all') {
      results = results.filter(([id, icon]) => icon.category === category);
    }
...
|---------|---------|------|`;
  ...
  generateIntegrationCode(icons) {
    ...
    let code = `## 💻 **Code d'intégration ready-to-use**
...
// Utilisation
${sampleIcons.map(icon => 
  `<DSFRIcon name="${icon.id}" title="${icon.description}" />`
).join('\n')}
\`\`\`
...
\`\`\``;
  ...
  getStatistics() {
    ...
    const categoriesStats = Object.entries(this.categories).map(([key, cat]) => ({
      ...
      percentage: Math.round((Object.values(this.icons).filter(icon => icon.category === key).length / totalIcons) * 100)
    }));
